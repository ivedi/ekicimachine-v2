app.angular
.component('machineFilter', {
    templateUrl: 'assets/angular/template/machine-filter.html?v=0.1.1',
    controller: ['$scope', '$http', '$window', function ($scope, $http, $window) {
        var ctrl = this;
        $scope.filter = {
            init: function () {
                $scope.filter.lang = document.documentElement.lang;
                $scope.filter.currentID = machineListUniData.categories.currentID;
                $scope.filter.list = machineListUniData.categories.list;
            },
            edit: {
                inProgress: false,
                show: function () {
                    $scope.filter.edit.init();
                    $('.machine-filter-edit-modal.modal:first').modal({ show: true, backdrop: false });
                },
                init: function () {
                    $scope.filter.edit.list = [];
                    for (var i = 0; i < machineListUniData.categories.list.length; i++) {
                        $scope.filter.edit.list.push(new app.edit.input(machineListUniData.categories.list[i].text[$scope.filter.lang], machineListUniData.categories.list[i].text[$scope.filter.lang], app.valid.text));
                    }
                },
                check: function () {
                    var apply = true;
            ...
            add: {
                ...
                show: function () {
                    ...
                    $('.machine-filter-add-modal.modal:first').modal({ show: true, backdrop: false });
                },
                init: function () {
            ...
            remove: {
                ...
                show: function () {
                    ...
                    $('.machine-filter-remove-modal.modal:first').modal({ show: true, backdrop: false });
                },
                init: function () {
                    ...
                    for (var i = 0; i < machineListUniData.categories.list.length; i++) {
                        $scope.filter.remove.list.push(new removeItem(machineListUniData.categories.list[i]));
                    }
                },
                submit: function () {
                    ...
                    for (var i = 0; i < machineListUniData.categories.list.length; i++) {
                        if ($scope.filter.remove.list[i].isRemoved) {
                            if (value.categories.list[i].id === $scope.filter.remove.list[i].category.id) {
                                value.list.forEach(function (machine) {
                                    machine.catIDs = machine.catIDs.filter(function (categoryID) {
